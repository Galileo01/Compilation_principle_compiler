...
const SyntaxAnalyseTab: React.FC<Props> = ({ tokenList = [] }) => {
  const [analyseResult, setResult] = useState<analyseResult>({ success: false, sponserOrder: [], T: [] });
  ...
  return (
    <>
      ...
      <Modal visible={treeVisible} onCancel={() => setVisible(false)} title="语法分析树" footer={null}>
        <SyntaxTree analyseSponser={analyseResult.sponserOrder} T={analyseResult.T} />
      </Modal>
    </>
  );
};
